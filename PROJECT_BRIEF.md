...
- **Scraping DBA and Bilbasen for price calibration is authorized as of 2026-09-02, at low, rate-limited volume ("within reason," the owner's words, citing an existing personal project that scrapes at roughly 35 listings/day without issue).** Reuse the existing adapter code and politeness settings (1-2s randomised delay, no concurrency, real browser UA) from the sibling `BestDeals` project rather than rebuilding. DBA's `robots.txt` does not disallow its general search/listing paths; Bilbasen's does (`Disallow: *?*` catches essentially all filtered search URLs), so treat DBA as primary and Bilbasen as secondary, and do not use Bilbasen's `/api/search/by-request` endpoint directly (`robots.txt` names `/api/` explicitly). Both sites' Terms of Service prohibit automated collection outright; this is a documented, deliberate risk the owner has accepted for this project, not an oversight. Biltorvet.dk was researched (see `BestDeals/BILTORVET-RESEARCH.md`) and is excluded: its Terms separately prohibit using the site to build a competing product, which this project's public output plausibly is. Scraped raw listings are never committed (same `data/raw/`-style gitignore treatment as DVSA/DMR); only aggregated price points land in version control.
